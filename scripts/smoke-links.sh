...
for link in "${LINKS[@]}"; do
    ...
    echo -n "link ${link} ... "
    if [[ "$link" == *nlogin.php* ]]; then
        echo "SKIP (optional module)"
        continue
    fi
    if ! curl -fsS -b "$SMOKE_COOKIE" -L --max-time 45 "$local_url" -o "$SMOKE_OUT" 2>/dev/null; then
        if [[ "$link" == *news.php* ]] || [[ "$link" == *nedit.php* ]]; then
            echo "SKIP (blog module optional)"
            continue
        fi
        echo "curl error"
        failed=1
